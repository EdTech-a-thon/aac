-- Snippet Inclusions: live placements of a Snippet on a host Board (or Snippet) row.
create table public.snippet_inclusions (
  id uuid primary key default gen_random_uuid(),
  host_id uuid not null references public.boards (id) on delete cascade,
  snippet_id uuid not null references public.boards (id) on delete cascade,
...
create index snippet_inclusions_host_id_idx on public.snippet_inclusions (host_id);
create index snippet_inclusions_snippet_id_idx on public.snippet_inclusions (snippet_id);

create trigger snippet_inclusions_set_updated_at
  before update on public.snippet_inclusions
  for each row execute function private.set_updated_at();

alter table public.snippet_inclusions enable row level security;

create policy "Managers can select snippet inclusions"
  on public.snippet_inclusions for select to authenticated
  using (
    private.is_vocabulary_manager(
      (select b.vocabulary_id from public.boards b where b.id = snippet_inclusions.host_id)
    )
  );

create policy "Managers can insert snippet inclusions"
  on public.snippet_inclusions for insert to authenticated
  with check (
    private.is_vocabulary_manager(
      (select b.vocabulary_id from public.boards b where b.id = host_id)
    )
  );

create policy "Managers can update snippet inclusions"
  on public.snippet_inclusions for update to authenticated
  using (
    private.is_vocabulary_manager(
      (select b.vocabulary_id from public.boards b where b.id = snippet_inclusions.host_id)
    )
...
create policy "Managers can delete snippet inclusions"
  on public.snippet_inclusions for delete to authenticated
  using (
    private.is_vocabulary_manager(
      (select b.vocabulary_id from public.boards b where b.id = snippet_inclusions.host_id)
    )
...
create policy "Communicators can select snippet inclusions"
  on public.snippet_inclusions for select to authenticated
  using (
...
grant select, insert, update, delete on public.snippet_inclusions to authenticated;
...
begin
  ...
  if op = 'create_snippet_inclusion' then
    host_id := (p_mutation ->> 'host_id')::uuid;
    snippet_id := (p_mutation ->> 'snippet_id')::uuid;
    origin_row := (p_mutation ->> 'origin_row')::integer;
    origin_col := (p_mutation ->> 'origin_col')::integer;
    if not exists (
      select 1 from public.boards b
      where b.id = host_id and b.vocabulary_id = p_vocabulary_id
    ) then
      raise exception 'Host not found for Snippet Inclusion';
    end if;
    if not exists (
      select 1 from public.boards b
      where b.id = snippet_id
    ...
    insert into public.snippet_inclusions (
      ...
      entity_id, host_id, snippet_id, origin_row, origin_col
    );
...
begin
  ...
  loop
    ...
    elsif op in ('create_snippet_inclusion', 'update_snippet_inclusion', 'delete_snippet_inclusion') then
      perform private.apply_snippet_inclusion_mutation(p_vocabulary_id, mutation);
