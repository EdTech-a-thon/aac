-- Symbols: a Button carries an immutable content digest naming the image shown on it.
-- The bytes themselves are uploaded outside Change Sets and are not Vocabulary data (ADR 0008).

alter table public.buttons
  add column symbol_digest text;

alter table public.buttons
  add constraint buttons_symbol_digest_check
  check (symbol_digest is null or symbol_digest ~ '^[0-9a-f]{64}$');

-- Symbol bytes live here, one object per distinct digest, shared by every
-- Vocabulary that references it. Public-read: the digest is the read
-- capability (ADR 0008). No insert/update/delete policy is defined, so only the
-- service role can write, and upload validation cannot be bypassed.
...
begin
  ...
  loop
    ...
    if op = 'create_board' then
      ...
      has_symbol := mutation ? 'symbol_digest';
      v_symbol_digest := null;
      if has_symbol and mutation -> 'symbol_digest' is not null
         and jsonb_typeof(mutation -> 'symbol_digest') <> 'null' then
        v_symbol_digest := lower(mutation ->> 'symbol_digest');
        if v_symbol_digest !~ '^[0-9a-f]{64}$' then
          raise exception 'Button symbol_digest must be a 64-character hex digest or null';
        end if;
      end if;
      insert into public.buttons (
        id, board_id, row_index, col_index, label, background_color, palette_color_id, action, symbol_digest
      )
      values (
        entity_id, board_id, row_index, col_index, label, v_background_color, v_palette_color_id, button_action, v_symbol_digest
      );
      ...
      set
        symbol_digest = case
          when has_symbol then v_symbol_digest
          else btn.symbol_digest
        end,
        row_index = case
