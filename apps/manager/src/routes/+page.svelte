...
<style>
	...
	.hero{min-height:640px;padding:55px 0 90px;display:grid;grid-template-columns:.9fr 1.1fr;gap:50px;align-items:center}
	/* The board's fixed width would otherwise set each column's automatic
	   minimum, widening the hero past a narrow viewport. */
	.hero>*{min-width:0}.kicker{color:var(--green);font-size:12px;font-weight:850;letter-spacing:.14em;text-transform:uppercase}.copy>.kicker{display:flex;align-items:center;gap:9px}.copy>.kicker span{width:25px;height:2px;background:var(--coral)}
	...
	/* Stacked on one column the scene's circle reaches up behind the copy, so the
	   copy keeps its own layer above it. */
